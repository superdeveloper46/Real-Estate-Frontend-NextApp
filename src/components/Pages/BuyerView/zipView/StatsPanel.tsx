...
const StatsPanel = (props: { data: any; loading: boolean }) => {
    ...
    (): FeatureCollection => ({
      ...
      features:
        props?.data?.map((dataPoint: any, i: number) => ({
          ...
          geometry: {
            type: 'Point',
            coordinates: [dataPoint[16], dataPoint[15]],
          },
          index: i,
        })) ?? [],
    }),
  ...
  useEffect(() => {
    ...
    setTableData([
      ['Seller Name', props?.data[Number(selectedMark)][13]],
      ['Sale Amount', props?.data[Number(selectedMark)][11]],
      ['Recording Date', props?.data[Number(selectedMark)][0]],
      ['Sale Date', props?.data[Number(selectedMark)][12]],
      ['Address', props?.data[Number(selectedMark)][5]],
      ['City', props?.data[Number(selectedMark)][6]],
      ['Zip', props?.data[Number(selectedMark)][7]],
      ['Bedrooms', props?.data[Number(selectedMark)][21]],
      ['Bathrooms', props?.data[Number(selectedMark)][22]],
    ]);
  }, [selectedMark]);
      ...
      <div className='relative w-full'>
        ...
        {selectedMark !== '' && props?.data?.[Number(selectedMark)] && (
          <div className='rad absolute right-[75px] top-1 z-[1000] w-[400px] overflow-hidden rounded-md bg-white'>
            ...
            >
              <p className='pl-3 text-xs font-bold'>
                {props.data[Number(selectedMark)][1]}
              </p>
              <IconButton>
                <svg
                  aria-hidden='true'
